...
<div class="form-field" id="${field.name}-control" style="display:none">
	...
	<input id="components-tohide" type="hidden" name="-" value="${hideProperties}"/>
      <input class="formsCheckBox" id="${fieldHtmlId}-entry" type="checkbox" tabindex="0" name="-" 
             value="true"
             onchange="PDFToolkit.Util.HideDependentControls('${fieldHtmlId}', '${args.htmlid}')" checked/>
      <label for="${fieldHtmlId}-entry" class="checkbox">${field.label?html}</label>
	<#--<script type="text/javascript">
           PDFToolkit.Util.HideMyControls('components-tohide', '${args.htmlid}');
        </script>-->
	</#if>
</div>
